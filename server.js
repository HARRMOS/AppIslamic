...
// Middleware pour vérifier l'authentification (session ou JWT)
function isAuthenticated(req, res, next) {
  // Vérifier d'abord si c'est une authentification par session
...
app.get('/auth/status', async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    console.log('Utilisateur authentifié par session, ID:', req.user.id);
    ...
    console.log('/auth/status - Envoi de la réponse user (authentifié par session): ', responseUser);
  ...
  if (token) {
    try {
      ...
      const activatedBots = getActivatedBotsForUser(decoded.id);
      console.log('/auth/status - Bots activés pour l\'utilisateur:', activatedBots);
      const responseUser = { 
        id: decoded.id, 
        name: decoded.name, 
...
// Route pour générer un token JWT (pour les tests ou l'authentification directe)
